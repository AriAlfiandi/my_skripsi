...
# ========== OPTIMASI FPS - TURUNKAN RESOLUSI ==========
cap.set(cv2.CAP_PROP_FRAME_WIDTH, 640)   # Width: 640px (default: 1280px)
cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 480)  # Height: 480px (default: 720px)
cap.set(cv2.CAP_PROP_FPS, 30)            # Target FPS: 30
print(f"✓ Resolusi kamera diset: 640x480 @ 30 FPS")
# ======================================================
detector = HandDetector(detectionCon=0.3, maxHands=1)

# Testing modes
testing_mode = None  # None, 'precision', 'smoothness', 'edge', 'rapid'
test_targets = []
...
class CursorTestingData:
    ...
    def add_precision(self, target_pos, cursor_pos, time_ms):
        ...
        hit = deviation < 30  # Hit jika < 30px
    ...
    def add_edge(self, edge_point, cursor_pos, time_ms):
        ...
        # Check if cursor reached edge (within 50px of edge)
        at_edge = (cursor_pos[0] < 50 or cursor_pos[0] > screen_width - 50 or
                   cursor_pos[1] < 50 or cursor_pos[1] > screen_height - 50)
    ...
    def add_rapid(self, start_pos, end_pos, time_ms):
        ...
        speed = (distance / time_ms * 1000) if time_ms > 0 else 0  # px/s
    ...
    def get_smoothness_stats(self):
        ...
        smoothness_score = max(0, 100 - jitter * 2)  # Score based on jitter
    ...
    def get_rapid_stats(self):
        ...
        # Accuracy based on successful rapid movements (> 200px in < 500ms)
        successful = sum(1 for d in self.rapid_data if d['distance'] > 200 and d['time_ms'] < 500)
    ...
    def save_report(self, filename='laporan_pengujian_kontrol_kursor_lengkap.txt'):
        ...
        with open(filename, 'w', encoding='utf-8') as f:
            ...
            if rapid:
                f.write(f"Rapid Movement | {rapid['count']} titik | {rapid['accuracy']:.2f} | {rapid['avg_time_ms']:.2f} | - | Gerakan cepat\n")
            else:
    ...
    if mode == 'precision':
        ...
        for row in range(3):
            for col in range(3):
                ...
                targets.append((x, y, 15))  # radius 15px (small)
    
    elif mode == 'edge':
        # 9 edge points (corners, sides, centers)
        edge_margin = 30
        targets = [
            (edge_margin, edge_margin, 20),  # Top-left
            (width // 2, edge_margin, 20),    # Top-center
            (width - edge_margin, edge_margin, 20),  # Top-right
            (edge_margin, height // 2, 20),   # Left-center
            (width - edge_margin, height // 2, 20),  # Right-center
            (edge_margin, height - edge_margin, 20),  # Bottom-left
            (width // 2, height - edge_margin, 20),   # Bottom-center
            (width - edge_margin, height - edge_margin, 20),  # Bottom-right
            (width // 2, height // 2, 20),    # Center
        ]
    
    elif mode == 'rapid':
        # 9 distant pairs for rapid movement
        targets = [
            (width // 4, height // 4, 25),
...
# Main loop variables
window_name = "Cursor Testing - 4 Aspek Pengujian"
...
print("\n" + "="*80)
print("PENGUJIAN KONTROL KURSOR - 4 ASPEK".center(80))
print("="*80)
print("\nMODE TESTING:")
...
while True:
    ...
    # Draw targets if in a test mode
    ...
    if len(hands) == 1:
        ...
        if lmList:
            ...
            # Smoothness tracking (continuous in mode 2)
    ...
    if key == ord('1'):
        ...
        current_target_index = 0
        print("\n✓ Mode: Precision Pointing (9 target kecil)")
        print("  Arahkan kursor ke target, tekan P untuk record")
        ...
        print("  Gerakkan kursor dengan halus, otomatis tercatat")
    
    elif key == ord('3'):
        testing_mode = 'edge'
        test_targets = generate_targets('edge', img.shape)
        current_target_index = 0
        print("\n✓ Mode: Edge Detection (9 titik edge)")
        print("  Arahkan kursor ke edge points, tekan P untuk record")
        ...
        rapid_start_pos = None
        print("\n✓ Mode: Rapid Movement")
        print("  Tekan P di start, gerak cepat, tekan P di end")
    ...
    # Record point
        ...
        if len(hands) == 1 and testing_mode and test_targets:
            ...
            if testing_mode == 'precision':
                ...
                print(f"  Target {current_target_index+1}: {status} (Deviation: {dev:.1f}px, Latency: {frame_time_ms:.1f}ms)")
                current_target_index += 1
                
                if current_target_index >= len(test_targets):
                    stats = test_data.get_precision_stats()
                    print(f"\n✅ Precision Test Complete!")
                    print(f"   Akurasi: {stats['accuracy']:.2f}%")
                ...
                print(f"  Edge {current_target_index+1}: {status} (Deviation: {dev:.1f}px, Latency: {frame_time_ms:.1f}ms)")
                current_target_index += 1
                ...
                if rapid_start_pos is None:
                    ...
                    print(f"  Start position recorded, gerak cepat ke target berikutnya!")
                else:
                    elapsed = (time.time() - rapid_start_time) * 1000
                    dist, speed = test_data.add_rapid(rapid_start_pos, (x3, y3), elapsed)
                    print(f"  Movement {current_target_index+1}: {dist:.1f}px in {elapsed:.1f}ms ({speed:.1f}px/s)")
                    rapid_start_pos = None
                    current_target_index += 1
                    ...
                    if current_target_index >= len(test_targets):
                        ...
                        print(f"   Akurasi: {stats['accuracy']:.2f}%")
                        print(f"   Avg Speed: {stats['avg_speed']:.1f}px/s")
                        current_target_index = 0
        ...
        if precision:
            ...
            print(f"   Count: {precision['count']}, Akurasi: {precision['accuracy']:.2f}%, Latency: {precision['avg_latency']:.2f}ms")
        
        smoothness = test_data.get_smoothness_stats()
        if smoothness:
            print(f"\n2. Cursor Smoothness:")
            print(f"   Count: {smoothness['count']}, Score: {smoothness['smoothness_score']:.2f}/100, Latency: {smoothness['avg_latency']:.2f}ms")
        
        edge = test_data.get_edge_stats()
        if edge:
            print(f"\n3. Edge Detection:")
            print(f"   Count: {edge['count']}, Akurasi: {edge['accuracy']:.2f}%, Latency: {edge['avg_latency']:.2f}ms")
        
        rapid = test_data.get_rapid_stats()
        if rapid:
            print(f"\n4. Rapid Movement:")
            print(f"   Count: {rapid['count']}, Akurasi: {rapid['accuracy']:.2f}%, Latency: {rapid['avg_time_ms']:.2f}ms")
