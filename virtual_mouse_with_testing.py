...
# ============= SISTEM TRACKING PENGUJIAN =============
class GestureTracker:
    def __init__(self):
        self.stats = {
            'scroll_up': {'detected': 0, 'true_positive': 0, 'false_positive': 0, 'false_negative': 0},
            'scroll_down': {'detected': 0, 'true_positive': 0, 'false_positive': 0, 'false_negative': 0},
            'click': {'detected': 0, 'true_positive': 0, 'false_positive': 0, 'false_negative': 0},
            'toggle_scroll': {'detected': 0, 'true_positive': 0, 'false_positive': 0, 'false_negative': 0},
            'move_cursor': {'detected': 0, 'true_positive': 0, 'false_positive': 0, 'false_negative': 0}
        }
    ...
    def gesture_detected(self, gesture_type):
        """Mencatat gesture yang terdeteksi"""
    ...
    def save_report_text(self, filename='laporan_pengujian.txt'):
        ...
        with open(filename, 'w', encoding='utf-8') as f:
            ...
            if overall_accuracy >= 90:
                ...
                f.write("Sistem memerlukan perbaikan untuk meningkatkan akurasi.\n")
            
            f.write("\nGesture dengan akurasi tertinggi:\n")
            best_gesture = max(self.stats.items(), 
                             key=lambda x: self.calculate_accuracy(x[0]))
            f.write(f"  - {best_gesture[0].replace('_', ' ').title()}: "
                   f"{self.calculate_accuracy(best_gesture[0]):.2f}%\n")
            
            f.write("\nGesture yang perlu ditingkatkan:\n")
            worst_gesture = min(self.stats.items(), 
                              key=lambda x: self.calculate_accuracy(x[0]))
            f.write(f"  - {worst_gesture[0].replace('_', ' ').title()}: "
                   f"{self.calculate_accuracy(worst_gesture[0]):.2f}%\n")
            
        print(f"✓ Laporan text disimpan ke: {filename}")
...
gesture_cooldown = 0.5  # 0.5 detik cooldown

# Variabel untuk tracking gesture sebelumnya
previous_scroll_state = None
previous_click_state = False
previous_fist_state = False

print("\n" + "="*80)
print("SISTEM PENGUJIAN VIRTUAL MOUSE - HAND TRACKING".center(80))
print("="*80)
print("\nPETUNJUK PENGGUNAAN:")
print("-" * 80)
print("KONTROL GESTURE:")
print("  • Gerakkan jari tengah    : Menggerakkan kursor")
print("  • Jempol-Telunjuk dekat   : Scroll ke atas")
print("  • Jempol-Telunjuk jauh    : Scroll ke bawah")
print("  • Jempol-Kelingking dekat : Klik kiri")
print("  • Kepal tangan            : Toggle scroll on/off")
print("  • ANGKAT KEDUA TANGAN (3 detik) : Keluar program")
print()
print("TOMBOL KONFIRMASI PENGUJIAN:")
print("  [Y] = True Positive  (Deteksi BENAR)")
print("  [N] = False Positive (Deteksi SALAH)")
print("  [F] = False Negative (Gesture tidak terdeteksi)")
print()
print("TOMBOL LAINNYA:")
print("  [T] = Toggle Scroll ON/OFF (LEBIH MUDAH!)")
print("  [W] = Matikan/Nyalakan SEMUA Fitur (Scroll + Klik)")
print("  [S] = Tampilkan Statistik")
print("  [R] = Reset Statistik")
print("  [Q] = Keluar & Simpan Laporan")
print("="*80 + "\n")

# Buat window dengan property always on top
window_name = "Virtual Mouse - Testing Mode"
...
while True:
    ...
    if check_both_hands_raised(hands):
        ...
        # Tampilkan countdown di layar (warna merah, besar)
        countdown = max(0, BOTH_HANDS_RAISED_DURATION - duration)
        cv2.putText(img, f"KELUAR DALAM: {countdown:.1f}s", (img.shape[1]//2 - 150, img.shape[0]//2), 
                   cv2.FONT_HERSHEY_SIMPLEX, 1.2, (0, 0, 255), 3)
        
        # Jika sudah 3 detik, keluar dari program
        if duration >= BOTH_HANDS_RAISED_DURATION:
            print("👋 Keluar program... Menyimpan data...")
            # Tampilkan statistik terakhir sebelum keluar
    ...
    if len(hands) == 1:
        ...
        if lmList:
            ...
            # Track move cursor (setiap detik)
            if current_time - last_gesture_time > 1.0:
                tracker.gesture_detected('move_cursor')
            ...
            if is_fist and not previous_fist_state:
                if current_time - last_gesture_time > gesture_cooldown:
                    ...
                    print(f"\n🤛 TOGGLE SCROLL: {'AKTIF' if scroll_active else 'NONAKTIF'}")
                    tracker.gesture_detected('toggle_scroll')
                    last_gesture_time = current_time
            ...
            if scroll_active and all_gestures_active:
                if length < 50:
                    ...
                    if previous_scroll_state != 'up':
                        if current_time - last_gesture_time > gesture_cooldown:
                            print("\n⬆️  SCROLL UP")
                            tracker.gesture_detected('scroll_up')
                            last_gesture_time = current_time
                    ...
                    if previous_scroll_state != 'down':
                        if current_time - last_gesture_time > gesture_cooldown:
                            print("\n⬇️  SCROLL DOWN")
                            tracker.gesture_detected('scroll_down')
                            last_gesture_time = current_time
                else:
            ...
            if current_click_state and not previous_click_state and all_gestures_active:
                if current_time - last_gesture_time > gesture_cooldown:
                    ...
                    print("\n👆 KLIK")
                    tracker.gesture_detected('click')
                    last_gesture_time = current_time
    ...
    info_text = f"Hands: {hands_count} | Gestures: {gesture_status} | Scroll: {'ON' if scroll_active else 'OFF'} | Akurasi: {tracker.calculate_overall_accuracy():.1f}%"
    ...
    if tracker.waiting_for_confirmation:
        cv2.putText(img, "Konfirmasi: Y=Benar N=Salah", (10, 60), 
